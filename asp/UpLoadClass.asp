...
Class UpLoadClass
	...
	Private Sub Class_Initialize
		...
		m_Charset  = "gb2312"
		m_TotalSize= 0
		m_MaxSize  = 153600'150KB
		m_FileType = "jpg/gif"
		m_SavePath = ""
		m_AutoSave = 0
		Dim dtmNow : dtmNow = now()
		m_strDate  = Year(dtmNow)&"-" &Right("0"&Month(dtmNow),2) & "-" & Right("0" & Day(dtmNow),2) & "-" & Right("0" & Hour(dtmNow),2) & "-" & Right("0" & Minute(dtmNow),2) & "-"
				...
				if m_binItem.Size<>0 then
					...
					if m_AutoSave<>2 then
						...
						if intTemp=0 then
							...
							m_dicForm.Add "ServerFileName",strFnam
						end if
					end if
				else
